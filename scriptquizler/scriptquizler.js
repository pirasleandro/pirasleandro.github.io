...
$.get("./script.txt", {}, function (content) {
  init(content);
  scene = script[0][0];
  current = [0, 0];
  displayTitle();
...
function next() {
  ...
  $("#output").append(
    $("<div>")
      ...
      .addClass(
        showCues && (scene[currentLine + 1] || " ")[0].includes(actor)
          ? "cue"
          : ""
      )
  );
...
function displayTitle() {
  ...
  $("#output")
    ...
    .append(
      $("<h2>").text(
        `${i + 1}${String.fromCharCode(j + 65)} - ${sceneMeta[i].title}`
      )
    );
}
